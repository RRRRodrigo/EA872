...
void threadfun (char *keybuffer, int *control, int* init_game) //essa thread agora é o server
{
  int c;
 ...
 //printf("Tentando abrir porta 3001\n");
  ...
  connection_fd = accept(socket_fd, (struct sockaddr*)&client, &client_size); //trava até aceitar
  while ((*control) == 1) {
    recv(connection_fd, &(input_buffer), sizeof(int), 0);
    (*init_game) = 1;
    char ip_client[INET_ADDRSTRLEN];
    inet_ntop( AF_INET, &(client.sin_addr), ip_client, INET_ADDRSTRLEN );


    (*keybuffer) = (char) input_buffer;
...
void Teclado::init(int* init_game) {
  // Inicializa ncurses
  ...
  this->rodando = 1;
  std::thread newthread(threadfun, &(this->ultima_captura), &(this->rodando), init_game);
